"""
 ...
 Low level functions for the communication with Pieter Fourie's PLC Intelligent 
  Roof Controller system at the Xamidiura telescope (originally SuperWASP 
  South).
  
 As translated from the php script originally written by Pierre Maxted, Sep 
  2015
  
 <07/01/19> I have change the roof request command from the old 
  '@00RD0150000351*\r' to '@00RD0150000451*\r'. This should get the PLC to 
  return 4 words instead of 3. This 4th word should contain the information 
  about the tilt of the telescope. Tested and it works.
  
 <14/1/19> For the new plcd script, the serial connection to the plc is left
  open, instead of being opened/closed each time a command is sent.
...
fileHand = logging.FileHandler(
		filename = set_err_codes.LOGFILES_DIRECTORY+'plc_status.log', mode='a')
...
# Bits in D-memory location 100 for each command, updated for the new PLC \
#  controller
...
def check_port_open_try_reopen(port_name):
	"""
	Will check that the port specified by 'port_name' is open. If isn't, it 
	 will try to reopen the port. If for some reason the open port function
	 cannot completed, or it can and then the port still isn't open, a
	 critical error message will be logged.
	 
	PARAMETERS:
	
		port_name = the port that will be checked
		
	RETURN:
	
		port_name = if port is open the port will be returned
	...
	if open_bool == False:
		...
		try:
			port_name = rcf.open_plc_port()
		except:
			logger.critical('CANNOT REACH PLC')
	
		else:
			if port_name.is_open() == False:
				logger.critical('CANNOT REACH PLC')
				
			else:
				logger.info('PLC port reopened')
	return port_name


def open_plc_port():
	"""
		Will open a port to the PLC using various settings.
    
		RETURN:
			open_port = the port that is opened
	"""
	open_port = serial.Serial(port = PLC_COM_PORT, baudrate = PLC_BAUD_RATE,
		parity=PLC_PARITY, stopbits = PLC_STOP_BITS,
		bytesize = PLC_CHARACTER_LENGTH,timeout = PLC_PORT_TIMEOUT)

	return open_port
  
def plc_command_response_port_open(command, open_port):
	
	
	open_port = check_port_open_try_reopen(open_port)
...
def plc_command_response(command=PLC_Status_Request):
	"""
	Sends a single PLC command, returns the response or triggers error if there 
	 is any problem. PLC command must include terminating "*\r"
  
		PARAMETERS
	
			command = the command to send to the PLC. By default it is the 
				status check command.
	
		RETURN
			response = The response from sending the command, if it was a valid
				command. returns False if the command was not a valid command.
...
def plc_string_is_valid(plc_string):
	...
	"""
	if plc_string[0] == '@' and plc_string[:3] == '@00' and \
		plc_string[-2:] =='*\r' and len(plc_string[3:-4])>=2:
...
def plc_insert_fcs(plc_string):
	...
	if plc_string[:3] == '@00' and len(plc_string[3:-2])>=2 and \
		plc_string[-4:] =='00*\r':
		
		frame = plc_string[:-4]
		oldfcs = plc_string[-4:-2]
		terminator = plc_string[-2:]
		logger.debug('Frame: '+str(frame)+', Old fcs: '+str(oldfcs)+', '\
...
def plc_status_request_response_plc(plc_string):
	...
	if plc_string[-2:] == '*\r' and plc_string[:5] =='@00MS':
		...
		logger.debug('Frame: '+str(frame)+', fcs: '+fcs+', status: '+str(
			status))
...
def plc_status_tilt_status(plc_string):
	...
	** NEW ** for Xamidimura telescope
	...
	RETURN
		...
		Either the current tilt status as D-memory 153, or 255 as set in the 
			settings and error codes script
		
	"""

	if plc_string[-2:] == '*\r' and plc_string[0] == '@' and \
		plc_string[:7] =='@00RD00':
		
		frame = plc_string[:-4]
		fcs = plc_string[-4:-2]
		tilt_status = plc_string[19:23]
		logger.debug('Frame: '+str(frame)+', fcs: '+fcs+', tilt status: '+str(
			tilt_status))
...
def plc_status_comms_timeout(plc_string):
	...
	RETURN
		...
		Either the current power timeout, or 255 as set in the settings and 
			error codes script
	...
	if plc_string[-2:] == '*\r' and plc_string[:5] =='@00RD':
		...
		logger.debug('Frame: '+str(frame)+', fcs: '+fcs+', timeout: '+str(
			timeout))
...
def plc_status_status_code(plc_string):
	...
	RETURN:
		...
		Either:
		 - the status code as a decimal number. This will contain information 
			as to which bits are set
		 OR
...
def plc_status_end_code(plc_string):
	...
	if plc_string[-2:] == '*\r' and plc_string[:5] =='@00RD':
		...
		logger.debug('Frame: '+str(frame)+', fcs: '+fcs+', endcode hex: '+str(
			endcode))
...
def plc_status_write_error_message(end_code):
	...
	PARAMETERS:
		...
		end_code = integer decimal representation of the end_code from a 
			response. Note the original is in hex format
			
	RETURN:
	 
		message = message that corresponds to the error message. The Error code 
			messages are stated in the original plc manual are for the hex 
			format numbers. As such the dictionary key require the error code 
			to be in hex format
	...
	try:
		...
		message = set_err_codes.PLC_STATUS_WRITE_ERROR_MESSAGE[str(
			hex_end_code)]
	except KeyError:
...
def plc_status_error_message(error_code):
	...
	The error code is converted to hex, because this hex value can be directly 
	 compare to what is read from a response string. i.e. in the response string
	 '@00SCA322*\r', 'A3' represents the error code in hex format, which would 
	 be inputted in this code as 163.
	 
	 PARAMETERS:
	 
		error_code = integer decimal representation of the error code from a 
		 response. Note that the original error code is in hex format.
			
	 RETURN:
	 
		message = message that corresponds to the error message. The Error code
		 messages are stated in the original plc manual are for the hex format 
		 numbers. As such the dictionary key require the error code to be in 
		 hex format
...
def plc_data_error_message(end_code):
	...
	RETURN:
		...
		message = message that corresponds to the error message. The Error code 
		 messages are stated in the original plc manual are for the hex format 
		 numbers. As such the dictionary key require the error code to be in hex
		 format
	"""
	#"""
	hex_end_code = format(end_code,"02x").upper()
...
def int_bit_is_set(intNo, offset):
	"""
	Will look at the integer representation of a status given by 'int' has the 
	 bit (given by 'offset' set.
	 
	 e.g. Reading in the status code '0003' from a plc response string will 
	  produce 1 and 2 for bits 0 and 1 respectively, but 0 for all others.
	  
	Note if the plc response string was found to be invalid at some point and 
	 255 has been return then all bits 1 through to 8 will be set.
	...
	RETURN:
		...
		bool(ans) = True/Falae. ans would be 0 if not set or 1,2,4,8,16... 
		 depending on which bit is being checked. The bool() function turn this 
		 to True/False response, which means this finction can be used in 
		 terms of:
	...
	if isinstance(intNo, int) == False or intNo<0:
		...
		if isinstance(offset, int) == False or offset <0 or offset >15:
			raise ValueError('Second argument should be a positive integer, '\
				'betweeen 0 and 15 inclusive.')
		else:
			ans = intNo & (1 <<offset)
